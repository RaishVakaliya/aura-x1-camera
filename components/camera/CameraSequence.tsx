...
export default function CameraSequence() {
  ...
  const drawImageToCanvas = useCallback((img: HTMLImageElement) => {
    ...
    if (W < 768 || dstAR < 1.0) {
      const scale = Math.min(W / srcW, (H * 0.72) / srcH);
      dw = srcW * scale;
      dh = srcH * scale;
      dx = (W - dw) / 2;
      dy = (H - dh) / 2 - H * 0.04;
    ...
    const st = ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        if (progressLabelRef.current) {
          progressLabelRef.current.textContent = `${String(targetFrame + 1).padStart(3, '0')} / ${TOTAL_FRAMES}`;
        }
    ...
    >
      <div
        style={{
          ...
          right: 0,
          height: '90px',
          background: 'linear-gradient(to bottom, #191e23 0%, transparent 100%)',
          zIndex: 2,
          pointerEvents: 'none',
        }}
      />
      ...
      >
        <span className="label label--light">01 / Product Study</span>
      </div>
